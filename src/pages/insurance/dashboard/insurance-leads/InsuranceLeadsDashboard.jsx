...
export default function InsuranceLeadsDashboard() {
  ...
  async function fetchLeads() {
    try {
      ...
      const leadsRes = await fetch(`${BASE_URL}/api/insurance?orgId=${orgId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async function updateStatus(id, status) {
    try {
      ...
      await fetch(`${BASE_URL}/api/insurance/lead/${id}/status`, {
        method: "PUT",
        headers: {
  ...
  if (loading) return <div className="p-8">Loading insurance leads...</div>;

  return (
    <div className="p-8">
      {/* Title + Back */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold mb-6">Insurance Enquiries</h2>
